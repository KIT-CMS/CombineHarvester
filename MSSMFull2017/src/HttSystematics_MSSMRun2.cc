...
void AddMSSMRun2Systematics(CombineHarvester & cb, int control_region, bool zmm_fit, bool ttbar_fit, bool regional_jec, int era) {
...
/*  cb.cp().process({"TTH_SM125"}).AddSyst(cb, "pdf_Higgs_ttH","lnN",
  ...
  cb.cp().process(JoinStr({signal, {"TTT","TTJ","TT","VV","VVT","VVJ", "ZL", "ZJ", "ZTT", "ZLL", "W_rest", "ZJ_rest", "TTJ_rest", "VVJ_rest"}})).AddSyst(cb,
    "lumi_13TeV_$ERA", "lnN", SystMap<>::init(lumi_unc));
  ...
  //   References:
  //   Notes: Taken from HTTSM2017, Uncertainties regarding combination of 2016 and 2017 commented out
  //   - FIXME: References?
  // ##########################################################################
  ...
  // 2016 and 2017 correlated uncertainty
  //cb.cp()
  //  .channel({"tt"})
  //  .process(mc_processes)
  ...
  // References:
  // Notes:
  // - Copied and slightly adapted from SMHTT2017
  // - FIXME: Adapt for fake factor and embedding
  ...
  /* TODO: Do not use embedding for now, no correlated uncertainties between embedding and mc needed
  ...
  */
  ...
  /* TODO: Do not use correlated efficiencies for 2016 and 2017 for now
 ...
 /* Comment out old uncertainties for backward comparisons 
...
/*  cb.cp().process(ch::JoinStr({signal, {"TTT","TTJ","VVT","VVJ","ZL","ZJ","ZTT","W","VV","TT","ZLL","QCD"}}))
  ...
  /* TODO: Which JES should be used? log normal from 2016 or shape from 2017?
  // Jet energy scale
  // ----------------
  cb.cp().AddSyst(cb,
  ...
  /* TODO: b-tagging shape uncertainties from SMHTT2017 not used for now.
          Use log normal uncertainties from 2016.
  cb.cp()
      .channel({"et", "mt", "tt", "em"})
      .process(mc_processes)
  ...
  cb.cp().AddSyst(cb,
    "CMS_mistag_b_$ERA", "lnN", SystMapAsymm<channel,bin_id,process>::init
    ({"em"}, {11}, {"ZLL"}, 0.98,1.01)
    ({"em"}, {11}, {"ZTT"}, 0.97,1.04)
    ({"em"}, {11}, {ggH}, 0.98,1.02)
    ({"em"}, {12}, {"ZLL"}, 1.01,0.99)
  ...
  // TODO: Do not use correlated uncertainty for now.
  /*
  // MC + embedded correlated uncertainty
  ...
  */
  ...
  cb.cp()
      .channel({"et", "mt", "tt"})
      .process(JoinStr({signal, {"ZTT", "TTT", "TTL", "VVT", "VVL", "jetFakes"}}))
  ...
  //cb.cp()
  //    .channel({"et", "mt", "tt"})
  //    .process(JoinStr({signal, {"ZTT", "TTT", "TTL", "VVT", "VVL", "jetFakes"}, {"EMB"}}))
 ...
 if(ttbar_fit){
   ...
   cb.cp().process({"TTT","TTJ","TT","TTL"}).AddSyst(cb,
     "CMS_htt_tjXsec_13TeV","lnN", SystMap<>::init(1.06));
 }
  
  // Diboson and ttbar Normalisation - fully correlated
  cb.cp().process({"VV","VVT","VVJ","VVJ_rest","VVL","ST"}).AddSyst(cb,
    "CMS_htt_vvXsec_13TeV", "lnN", SystMap<>::init(1.05));

  // W norm, just for em, tt and the zmm region where MC norm is from MC
  // TODO: Added et and mt channel because no embedding is used
  cb.cp().process({"W","W_rest"}).channel({"tt","em","zmm","et","mt"}).AddSyst(cb,
  ...
  // TODO: QCD uncertainties from SM HTT 2017, not used yet. Maybe remeasure corrections for extrapolation.
  /*
  cb.cp()
      .channel({"et"})
      .process({"QCD"})
