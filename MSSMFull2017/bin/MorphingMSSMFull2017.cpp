...
#include "CombineHarvester/MSSMFull2017/interface/HttSystematics_MSSMRun2.h"
...
int main(int argc, char** argv) {
  ...
  string base_path = string(getenv("CMSSW_BASE")) + "/src/CombineHarvester/MSSMFull2017/shapes/";
  ...
  string sm_gg_fractions = string(getenv("CMSSW_BASE")) + "/src/CombineHarvester/MSSMFull2016/shapes/Models/higgs_pt_v3.root";
  string chan;
  po::variables_map vm;
  ...
  std::cout << "ggHatNLO? " << ggHatNLO << std::endl;
  std::cout << "mass? " << mass << std::endl;
  typedef vector<string> VString;
  typedef vector<pair<int, string>> Categories;
  std::map<string, string> input_dir;
  input_dir["zmm"]  = string(getenv("CMSSW_BASE")) + "/src/CombineHarvester/MSSMFull2017/shapes/"+input_folder_zmm+"/";
  input_dir["ttbar"]  = string(getenv("CMSSW_BASE")) + "/src/CombineHarvester/MSSMFull2017/shapes/"+input_folder_em+"/";
  ...
  if(!bbH_nlo){
    ...
    input_dir["em"]  = base_path + input_folder_em+"/NLO/";
    input_dir["mt"]  = base_path + input_folder_mt+"/NLO/";
    input_dir["et"]  = base_path + input_folder_et+"/NLO/";
    input_dir["tt"]  = base_path + input_folder_tt+"/NLO/";
  }
  std::cout << "tt input directory: " << input_dir["tt"] << std::endl;
  ...
  if (do_jetfakes){
    bkg_procs["et"] = {"ZL", "TTT","TTL","VVT","VVL","ZTT","jetFakes"};
    bkg_procs["mt"] = {"ZL", "TTT","TTL","VVT","VVL","ZTT","jetFakes"};
    bkg_procs["tt"] = {"ZL", "TTT","TTL","VVT","VVL","ZTT","jetFakes"};
  }else{
    bkg_procs["et"] = {"W", "QCD", "ZL", "ZJ", "TTT", "TTJ", "TTL", "VVT","VVJ", "VVL", "ZTT"};
    bkg_procs["mt"] = {"W", "QCD", "ZL", "ZJ", "TTT", "TTJ", "TTL", "VVT","VVJ", "VVL", "ZTT"};
    bkg_procs["tt"] = {"W", "QCD", "ZL", "ZJ", "TTT", "TTJ", "TTL", "VVT","VVJ", "VVL", "ZTT"};
  }
...
/*  binning["zmm_nobtag"] = {60,70,80,90,100,110,120};
 ...
 cats["ttbar_Run2017"] = {
  ...
  if (control_region > 0){
      // for each channel use the categories >= 14 for the control regions
      // the control regions are ordered in triples (14,15,16),(17,18,19)...
      for (auto chn : chns){
        ...
        Categories queue;
        int binid = 14;
        for (auto cat:cats[chn+"_Run2017"]){
        ...
        cats[chn+"_Run2017"].insert(cats[chn+"_Run2017"].end(),queue.begin(),queue.end());
      }
  }
  ...
  vector<string> masses = {/*"80", */"90","100","110","120","130","140","180", "200", "250", "350", "400", "450", "600", "700", "800", "900","1200","1400"/*,"1500"*/,"1600","1800","2000","2300","2600","2900","3200"};
  ...
  std::cout << "Trying to AddObservations and Proceses to CombineHarvester." << std::endl;
  vector<string> sig_procs = {"ggH","bbH"};
  for(auto chn : chns){
    cb.AddObservations({"*"}, {"htt"}, {"Run2017"}, {chn}, cats[chn+"_Run2017"]);

    cb.AddProcesses({"*"}, {"htt"}, {"Run2017"}, {chn}, bkg_procs[chn], cats[chn+"_Run2017"], false);

    cb.AddProcesses(masses, {"htt"}, {"Run2017"}, {chn}, signal_types["ggH"], cats[chn+"_Run2017"], true);
    cb.AddProcesses(masses, {"htt"}, {"Run2017"}, {chn}, signal_types["bbH"], cats[chn+"_Run2017"], true);
    if(SM125==string("bkg_SM125") && chn!="zmm") cb.AddProcesses({"*"}, {"htt"}, {"Run2017"}, {chn}, SM_procs, cats[chn+"_Run2017"], false);
    if(SM125==string("signal_SM125") && chn!="zmm") cb.AddProcesses({"*"}, {"htt"}, {"Run2017"}, {chn}, SM_procs, cats[chn+"_Run2017"], true);
    }
  ...
  std::cout << "Try to add systematics." << std::endl;
  ...
  std::cout << "Delete processes with 0 yield." << std::endl;
...
/*     cb.cp().process({"TTH_SM125"}).ForEachProc([&](ch::Process *proc){
  ...
  std::cout << "Filter for systs." << std::cout;
  ...
  if(postfit_plot){
    cb.ForEachSyst([](ch::Systematic *s) {
      if (s->type().find("shape") == std::string::npos) return;
      if (s->name().find("CMS_htt_tt_btag_Run2017_ZTT_bin_18") == std::string::npos) return;
  ...
  /*cb.cp().syst_name({"CMS_scale_j_Run2017"}).ForEachSyst([](ch::Systematic *sys) { sys->set_type("lnN");});
  cb.cp().syst_name({"CMS_scale_b_Run2017"}).ForEachSyst([](ch::Systematic *sys) { sys->set_type("lnN");});
  cb.cp().syst_name({"CMS_fake_b_Run2017"}).ForEachSyst([](ch::Systematic *sys) { sys->set_type("lnN");});*/
  ...
  std::cout << "About to start morphing." << std::endl;
  if (do_morphing) {
  ...
  std::cout << "Closing demo.." << std::endl;
  demo.Close();
  std::cout << "Demo closed.." << std::endl;
  cb.AddWorkspace(ws);
  std::cout << "Workspace added.." << std::endl;
  //ws.Print();
 ...
 std::cout << "Setup for writing datacards.." << std::endl;
 ...
 //note that it's also possible to write out the full combined card with CH
  ...
  std::cout << "About to write datacards..." << std::endl;
